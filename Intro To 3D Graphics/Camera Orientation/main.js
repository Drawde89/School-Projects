...
function initTextures() {
    ...
    textureArray.push({}) ;
    loadFileTexture(textureArray[textureArray.length-1],"stars.jpg") ;
...
function render() {
    ...
    gl.uniform1i(gl.getUniformLocation(program, "texture3"), 2);
      ...
      console.log(TIME);
        ...
        gPush();{
          toggleTextures(2);
          eye=vec3(TIME*3-5,0,-10);
          at = vec3(TIME * 3-5, 0, -10)
          gTranslate((TIME*3) - 5, 2*(Math.sin(TIME)) ,0);
          drawCube() ;
          }
            gPop();
          }
        ...
        if(TIME >= 5.0 && TIME < 9.0){
            ...
            gPush();{
              ...
              gTranslate(50, 2* (Math.sin(TIME)), 0);
              drawSphere();
            }
            gPop();
          }
        ...
        if(TIME >= 9  && TIME < 13.2){
          ...
          gPush();{
          toggleTextures(2);
          gTranslate(start, (Math.sin(TIME)), 0);
          drawCube() ;
            if(TIME > 12.5){
              gPush();{
              gTranslate(-2, 0, 0);
        ...
        if(TIME >= 13.2 && TIME < 19){
          eye=vec3(0,0,-10);
          at = vec3(0,0,0);
          start2 = start2 +.04;
          gPush();{
          toggleTextures(2);
          gTranslate(start2, (Math.sin(TIME)), 0);
          drawCube();
            gPush();{
              gTranslate(-2, 0, 0);
              drawSphere();
...
/*
function changeProgram(newProgram, shape){
  gl.useProgram( newProgram );


  // Load canonical objects and their attributes
  if(shape == "cube"){
    Cube.init(newProgram);
  }
  else if(shape == "cylinder"){
    Cylinder.init(9,newProgram);
  }else if (shape == "cone") {
    Cone.init(9,newProgram) ;
  }else if(shape == "sphere"){
    Sphere.init(36,newProgram) ;
  }else{
  ...
  // record the locations of the matrices that are used in the shaders
  modelViewMatrixLoc = gl.getUniformLocation( newProgram, "modelViewMatrix" );
  normalMatrixLoc = gl.getUniformLocation( newProgram, "normalMatrix" );
  projectionMatrixLoc = gl.getUniformLocation( newProgram, "projectionMatrix" );
  setColor(materialDiffuse) ;
